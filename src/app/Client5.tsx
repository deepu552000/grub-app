...
function ClientPageInner() {
  ...
  function applyIdentityLoad(s: PetState, identity: string | null, trusted: boolean) {
    console.log("[MOUNT] applyIdentityLoad() — setting hydrated=true for identity:", identity, "trusted:", trusted);
    hydratedRef.current = true;
  ...
  useEffect(() => {
    ...
    console.log("[MOUNT] DB-load effect firing for identity:", identityParam);
    ...
    async function loadWithRetries() {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          console.log(`[MOUNT] fetching /api/pet?${forIdentity} (attempt ${attempt})`);
          ...
          if (cancelled) {
            console.log("[MOUNT] fetch succeeded but effect was already cancelled (identity changed mid-flight) — discarding");
            return;
          }
          const dbState = saved ? loadStateFromSaved(saved) : { ...defaultState, lastVisit: Date.now() };
          console.log("[MOUNT] applying DB state, calling applyIdentityLoad");
          ...
          console.log(`[MOUNT] /api/pet attempt ${attempt} failed:`, e);
          lastErr = e;
    ...
    loadWithRetries();
    return () => {
      console.log("[MOUNT] DB-load effect cleanup — cancelling in-flight load for:", forIdentity);
      cancelled = true;
    };
  }, [identityParam]);
  ...
  useEffect(() => {
    ...
    // Absolute last-resort watchdog — unlike fallbackTimer above, this is
    // NEVER cleared just because sdk.context resolved. sdk.context
    // resolving only tells us whether we have a fid; it says nothing about
    // whether wallet identity detection (silentlyDetectWallet) will
    // actually finish. That detection races wagmi's OWN automatic
    // reconnect (triggered by ssr:true) against the manual reconnect()
    // call inside silentlyDetectWallet — both hit the same connector, and
    // watchAccount can observe the connector bouncing through intermediate
    // states (address -> undefined -> address) as a result. Each bounce
    // cancels the in-flight DB-load fetch ("identity changed mid-flight");
    // if the final bounce-back never lands cleanly, nothing is left to
    // ever call hydrateWith/applyIdentityLoad again, and the app is stuck
    // on "Loading Grub..." forever with no further recovery — exactly what
    // was observed on a direct browser load. This timer guarantees SOME UI
    // renders within 6s no matter what the identity race does. It's safe
    // even if a real identity load is still in flight: hydrateWith()
    // no-ops once hydrated is already true, and applyIdentityLoad() (used
    // by the real DB-load effect) has no such guard, so a genuine DB read
    // that lands after this fires still wins and replaces the local
    // fallback data.
    const hardFailsafeTimer = setTimeout(() => {
      if (!hydratedRef.current) {
        console.log("[MOUNT] hardFailsafeTimer fired — identity detection never settled, forcing hydrateWith(loadState())");
        hydrateWith(loadState());
      }
    }, 6000);
  ...
  // cash-out/deposit flow entirely; only config/bet/roll state is its own. ─
  ...
  const [diceConfig, setDiceConfig] = useState<{
    ...
    minWinChancePercent: number;
    maxWinChancePercent: number;
    maxMultiplier: number;
    maxPayoutDegen: number;
  } | null>(null);
  // Balance is SHARED with Coin Toss (same internal balance) — kept as its
  // own piece of state here so this panel works standalone even if the
  // Coin Toss panel hasn't been opened/fetched yet this session.
  const [diceBalance, setDiceBalance] = useState(0);
  const [diceRecentRolls, setDiceRecentRolls] = useState<Array<{
    target: number; direction: "under" | "over"; roll: number; won: boolean;
    winChancePercent: number; multiplier: number; betDegen: number; payoutDegen: number; ts: number;
    serverSeedHash: string; nonce: number; clientSeed: string;
  }>>([]);
  const [diceBet, setDiceBet] = useState(10);
  const [diceTarget, setDiceTarget] = useState(50); // 2–98, slider value
  const [diceDirection, setDiceDirection] = useState<"under" | "over">("under");
  const [diceRolling, setDiceRolling] = useState(false);
  const [diceLastResult, setDiceLastResult] = useState<{
  ...
  // in lib/minigames.ts's placeDiceBet, so the slider shows the player the
  // real numbers before they commit to a roll. Purely a preview: the
  // server always recomputes and enforces this itself.
  const diceWinChancePercent = diceDirection === "under" ? diceTarget - 1 : 100 - diceTarget;
  const diceRawMultiplier = diceConfig && diceWinChancePercent > 0
    ? (100 / diceWinChancePercent) * (1 - diceConfig.houseEdgePercent / 100)
    : 0;
  const diceMultiplier = diceConfig ? Math.min(diceRawMultiplier, diceConfig.maxMultiplier) : 0;
  const diceWinChanceOutOfRange = !!diceConfig &&
    (diceWinChancePercent < diceConfig.minWinChancePercent || diceWinChancePercent > diceConfig.maxWinChancePercent);
  const dicePotentialPayout = diceConfig ? Math.min(diceBet * diceMultiplier, diceConfig.maxPayoutDegen) : 0;

  const fetchDiceStatus = useCallback(async () => {
    try {
      const params = new URLSearchParams();
      if (fid) params.set("fid", String(fid));
      if (walletAddress) params.set("wallet", walletAddress);
      const res = await fetch(`/api/minigames/dice?${params.toString()}`).then((r) => r.json());
      if (res?.ok) {
  ...
  async function doDiceRoll() {
    ...
    try {
      const res = await fetch("/api/minigames/dice", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...identity, action: "roll", betDegen: diceBet, target: diceTarget, direction: diceDirection }),
      }).then((r) => r.json());

      if (!res?.ok) {
        setLastAction(`✕ ${res?.reason ?? "Roll failed"}`);
        setDiceRolling(false);
        return;
      }

      // Same cosmetic delay pattern as doCoinToss — lets a roll animation
      // play before the result reveals.
      setTimeout(() => {
        setDiceBalance(res.newBalance);
        setCointossBalance(res.newBalance); // same underlying balance — keep the Coin Toss panel in sync too
        setDiceLastResult({ roll: res.roll, target: res.target, direction: res.direction, won: res.won, payoutDegen: res.payoutDegen });
        setLastAction(
          res.won
            ? `🎲 Dice: rolled ${res.roll}! You won ${res.payoutDegen.toFixed(1)} DEGEN!`
            : `🎲 Dice: rolled ${res.roll}. Better luck next roll!`
        );
        fetchDiceStatus();
        setDiceRolling(false);
      }, 700);
    } catch (err: any) {
      setLastAction(`✕ ${err?.message ?? "Roll failed"}`);
      setDiceRolling(false);
    }
  }
  ...
  const fetchCointossStatus = useCallback(async () => {
    try {
      ...
      if (fid) params.set("fid", String(fid));
      if (walletAddress) params.set("wallet", walletAddress);
      const res = await fetch(`/api/minigames/cointoss?${params.toString()}`).then((r) => r.json());
      if (res?.ok) {
  ...
  useEffect(() => {
    if (!diceOpen) return;
    fetchDiceStatus();
    // Also pull Coin Toss status here — Dice's Deposit/Cash Out UI reuses
    // Coin Toss's balance/wallet/cash-out-queue state (same underlying
    // balance either way), so this needs to be populated even if the
    // player never opens the Coin Toss panel this session.
    fetchCointossStatus();
  }, [diceOpen, fetchDiceStatus, fetchCointossStatus]);

  // Poll while the Coin Toss panel OR the Dice panel is open — either one
  // can have a cash-out queued for the admin dashboard (anything over
  // autoCashoutMaxDegen), since they share the same queue.
  useEffect(() => {
    if (!cointossOpen && !diceOpen) return;
  ...
  }, [cointossOpen, diceOpen, cointossMyCashouts, fetchCointossStatus]);
  ...
  async function doCoinToss() {
    ...
    setCointossFlipping(true);
    setCointossLastResult(null);
    try {
      const res = await fetch("/api/minigames/cointoss", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...identity, action: "flip", betDegen: cointossBet, choice: cointossChoice }),
      }).then((r) => r.json());

      if (!res?.ok) {
        setLastAction(`✕ ${res?.reason ?? "Flip failed"}`);
        setCointossFlipping(false);
        return;
      }

      // Small delay so the flip animation has time to play before revealing
      // the result — purely cosmetic, matches the Spin Wheel's spin-then-reveal pacing.
      setTimeout(() => {
        setCointossBalance(res.newBalance);
        setDiceBalance(res.newBalance); // same underlying balance — keep the Dice panel in sync too result: res.result, won: res.won, payoutDegen: res.payoutDegen, serverSeedHash: res.serverSeedHash, nonce: res.nonce, clientSeed: res.clientSeed });
      ...
      setLastAction(`✕ ${err?.message ?? "Flip failed"}`);
      setCointossFlipping(false);
    }
  }
  ...
  async function doCointossCashout(sourceGame: "cointoss" | "dice" = "cointoss") {
    ...
    setCointossCashoutLoading(true);
    try {
      const res = await fetch("/api/minigames/cointoss", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (!res?.ok) {
        setLastAction(`✕ ${res?.reason ?? "Cash-out failed"}`);
        return;
      }
      setLastAction(
        res.status === "fulfilled"
          ? `✓ Cashed out ${cointossCashoutAmount} DEGEN — sent instantly.`
          : `✓ Cash-out requested — larger amounts are sent by an admin shortly.`,
      );
      setCointossCashoutAmount(0);
      fetchCointossStatus();
      fetchDiceStatus(); // same underlying balance — keep the Dice panel's balance in sync too
    } catch (err: any) {
  ...
  async function doCointossDeposit(sourceGame: "cointoss" | "dice" = "cointoss") {
    ...
    try {
      ...
      setCointossBalance(res.newBalance);
      setDiceBalance(res.newBalance); // same underlying balance — keep the Dice panel in sync too
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "center", padding: "8px 0", perspective: 600 }}>
                <div
                  style={{
                    ...
                    transition: cointossFlipping ? "transform 0.9s cubic-bezier(0.34, 1.56, 0.64, 1)" : "transform 0.4s ease",
                    transform: cointossFlipping
                      ? "rotateY(1440deg)"
                      : cointossLastResult
                        ? `rotateY(${cointossLastResult.result === "heads" ? 0 : 180}deg)`
                        : "rotateY(0deg)",
                  }}
                >
                  <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                ...
                <input
                  type="number"
                  value={cointossBet}
                  onChange={(e) => setCointossBet(Number(e.target.value))}
              ...
              >
                ...
                >
                  <span>🎲 Dice</span>
                  <span className="stats-chevron">{diceOpen ? "▲" : "▼"}</span>
                </button>
                ...
                {diceOpen && (
                  <div
                    ...
                    style={{ display: "flex", flexDirection: "column", gap: 12, padding: "14px 10px" }}
                  >
                    {!diceConfig?.enabled && diceConfig && (
                      <div style={{ textAlign: "center", fontSize: 12, color: "#92400e", fontWeight: 700, background: "#fef3c7", borderRadius: 8, padding: "8px 10px" }}>
                        ⏸ Dice is paused right now — check back soon.
                      </div>
                    )}

                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 12, color: "#49332d", fontWeight: 600 }}>
                      <span>Balance: <b style={{ color: "#b45309" }}>{diceBalance.toFixed(1)} DEGEN</b></span>
                      {diceConfig && <span>Bet: {diceConfig.minBetDegen}–{diceConfig.maxBetDegen}</span>}
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {(["under", "over"] as const).map((dir) => (
                        <button
                          key={dir}
                          type="button"
                          onClick={() => setDiceDirection(dir)}
                          style={{
                            flex: 1, padding: "8px 10px", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer",
                            border: diceDirection === dir ? "2px solid #b45309" : "1px solid #eee0d8",
                            background: diceDirection === dir ? "#fef3c7" : "#fff",
                            color: diceDirection === dir ? "#92400e" : "#49332d",
                          }}
                        >
                          Roll {dir === "under" ? "Under" : "Over"}
                        </button>
                      ))}
                    </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#49332d", fontWeight: 700 }}>
                        <span>Target: {diceTarget}</span>
                        <span>Win chance: {diceWinChancePercent}%</span>
                      </div>
                      <input
                        type="range"
                        min={2}
                        ...
                        style={{ width: "100%" }}
                      />
                      {diceWinChanceOutOfRange && diceConfig && (
                        <div style={{ fontSize: 11, color: "#b91c1c", fontWeight: 600 }}>
                          Win chance must be {diceConfig.minWinChancePercent}%–{diceConfig.maxWinChancePercent}% — adjust the slider.
                        </div>
                      )}
                    </div>

                    {/* Multiplier / potential payout preview */}
                    <div style={{ display: "flex", justifyContent: "space-between", background: "#fdf8f3", borderRadius: 8, padding: "8px 10px", fontSize: 12, fontWeight: 700, color: "#49332d" }}>
                      <span>Multiplier: {diceMultiplier.toFixed(2)}x</span>
                      <span>To win: {dicePotentialPayout.toFixed(1)} DEGEN</span>
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <input
                        ...
                        min={diceConfig?.minBetDegen ?? 1}
                        max={diceConfig?.maxBetDegen ?? 999999}
                        style={{ flex: 1, border: "1px solid #eee0d8", borderRadius: 8, padding: "8px 10px", fontSize: 13 }}
                      />
                      <span style={{ fontSize: 12, color: "#49332d", fontWeight: 600 }}>DEGEN</span>
                    </div>
                    {diceBetOutOfRange && diceConfig && (
                      <div style={{ fontSize: 11, color: "#b91c1c", fontWeight: 600, marginTop: -6 }}>
                        Bet must be between {diceConfig.minBetDegen} and {diceConfig.maxBetDegen} DEGEN.
                      </div>
                    )}
                    ...
                    {diceLastResult && (
                      <div style={{
                        ...
                        background: diceLastResult.won ? "#dcfce7" : "#fee2e2",
                        color: diceLastResult.won ? "#166534" : "#991b1b",
                      }}>
                        Rolled {diceLastResult.roll} ({diceLastResult.direction === "under" ? "<" : ">"} {diceLastResult.target}) —{" "}
                        {diceLastResult.won ? `Won ${diceLastResult.payoutDegen.toFixed(1)} DEGEN!` : "Lost"}
                      </div>
                    )}
                    ...
                    {/* Deposit / Cash Out — Dice shares Coin Toss's internal
                        ...
                        reuses the same state (cointossDepositAmount,
                        cointossCashoutWallet, cointossMyCashouts,
                        cointossMyDeposits, etc.) as the Coin Toss panel —
                        one wallet, one balance, one cash-out queue, no
                        matter which game panel the player uses to get
                        there. Small cash-outs (≤ autoCashoutMaxDegen) send
                        instantly; anything larger queues for an admin to
                        fulfill from the dashboard, exactly like Coin Toss. */}
                    <div style={{ borderTop: "1px solid #eee0d8", paddingTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                      <span style={{ fontSize: 11, color: "#8a7060", fontWeight: 700 }}>Deposit DEGEN from your wallet</span>
                      <div style={{ display: "flex", gap: 6 }}>
                        <input
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: paymentInFlight || cointossDepositAmount <= 0 ? "#d6d3d1" : "#7c3aed",
                            color: "#fff",
                            cursor: paymentInFlight || cointossDepositAmount <= 0 ? "default" : "pointer",
                          }}
                        >
                    ...
                    <div style={{ borderTop: "1px solid #eee0d8", paddingTop: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                      ...
                      {/* Same shared queue Coin Toss shows — pending cash-outs
                          from either game land in the same admin queue. */}
                      {cointossMyCashouts.length > 0 && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 2 }}>
                          {cointossMyCashouts.slice(0, 5).map((c) => {
                            const style =
...
function SuggestModal({
  ...
  // Tracks which tickets are expanded — tickets start collapsed, and any
  // with an unread admin reply auto-expand so the new message is visible
  // without an extra tap.
  const [expandedTicketIds, setExpandedTicketIds] = useState<Set<string>>(
    () => new Set(myTickets.filter((t) => t.unread).map((t) => t.id))
  );
  const onToggleTicketExpanded = (id: string) => {
        ...
        <div style={{ padding: "4px 20px 20px" }}>
          ...
          {type === "issue" && myTickets.length > 0 && (
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 16 }}>
              {myTickets.map((t) => {
                const isOpen = expandedTicketIds.has(t.id);
                const snippet = t.text.length > 60 ? `${t.text.slice(0, 60)}…` : t.text;
                return (
                <div key={t.id} style={{ border: "1px solid rgba(255,255,255,0.18)", borderRadius: 8, overflow: "hidden" }}>
                  <div
                    onClick={() => onToggleTicketExpanded(t.id)}
                    style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, padding: 10, cursor: "pointer" }}
                  >
                    <span
                      style={{
                        fontSize: 10,
                    ...
                    <span style={{ fontFamily: "monospace", fontSize: 10, color: "#000", opacity: 0.75, flexShrink: 0 }}>#{t.id.slice(-6)}</span>
                    <span style={{ fontSize: 10, color: "#000", opacity: 0.75, flexShrink: 0 }}>{new Date(t.ts).toLocaleDateString()}</span>
                    {!isOpen && (
                      <span style={{ fontSize: 11.5, opacity: 0.55, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", flex: 1, textAlign: "right" }}>
                        {snippet}
                      </span>
                    )}
                  </div>
